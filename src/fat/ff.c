...
#define	NAMEBUF(sp,lp)	UCHAR sp[12]; WCHAR *lp = LfnBuf
#define INITBUF(dj,sp,lp)	dj.fn = sp; dj.lfn = lp

#elif _USE_LFN > 1	/* LFN with dynamic LFN working buffer */
#define	NAMEBUF(sp,lp)	UCHAR sp[12]; WCHAR lbuf[_MAX_LFN + 1], *lp = lbuf
#define INITBUF(dj,sp,lp)	dj.fn = sp; dj.lfn = lp

#else				/* No LFN */
#define	NAMEBUF(sp,lp)	UCHAR sp[12]
...
#if _USE_LFN
void gen_numname (
	UCHAR *dst,			/* Pointer to genartated SFN */
...
)
{
	...
	*rfs = fs = FatFs[vol];			/* Returen pointer to the corresponding file system object */
	...
	fs->drive = (UCHAR)LD2PD(vol);		/* Bind the logical drive and a physical drive */
...
)
{
	...
	res = chk_mounted(&path, &dj.fs, (UCHAR)(mode & (FA_WRITE | FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)));
#else
	mode &= FA_READ;
	...
	for ( ;  btr;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {			/* On the sector boundary? */
			...
			if (cc) {								/* Read maximum contiguous sectors directly */
				...
				if (disk_read(fp->fs->drive, rbuff, sect, (UCHAR)cc) != RES_OK)
...
#endif
#endif
				fp->csect += (UCHAR)cc;				/* Next sector address in the cluster */
	...
	for ( ;  btw;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {			/* On the sector boundary? */
			...
			if (cc) {								/* Write maximum contiguous sectors directly */
				if (fp->csect + cc > fp->fs->csize)	/* Clip at cluster boundary */
					cc = fp->fs->csize - fp->csect;
				if (disk_write(fp->fs->drive, wbuff, sect, (UCHAR)cc) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
#if _FS_TINY
...
)
{
	...
	if (ofs > 0) {
		...
		if (clst != 0) {
			...
			fp->fptr += ofs;
			fp->csect = (UCHAR)(ofs / SS(fp->fs));	/* Sector offset in the cluster */
			if (ofs % SS(fp->fs)) {
...
FRESULT f_mkfs (
	...
	UCHAR partition,		/* Partitioning rule 0:FDISK, 1:SFD */
...
)
{
	...
	tbl[BPB_SecPerClus] = (UCHAR)allocsize;		/* Sectors per cluster */
	...
	m = (UCHAR)((fmt == FS_FAT32) ? allocsize : n_dir);
	do {
		if (disk_write(drv, tbl, b_fat++, 1) != RES_OK)
			return FR_DISK_ERR;
	} while (--m);
